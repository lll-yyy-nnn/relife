...
class App {
    ...
    initPages () {
        ...
        <div id="main">
            ...
            <div id="title">
            超级穷鬼系统<br>
                <div style="font-size:1.5rem; font-weight:normal;">这垃圾超级穷鬼系统也不想呆了</div>
            </div>
            <button id="restart" class="mainbtn"><span class="iconfont">&#xe6a7;</span>立即开启超级穷鬼系统</button>
        ...
        const talentPage = $(`
        <div id="main">
            <div class="head" style="font-size: 1.6rem">穷鬼天赋抽卡</div>
                    ...
                    .forEach(talent => {
                        ...
                        li.click(() => {
                            if (li.hasClass('selected')) {
                                ...
                                if (this.#talentSelected.size == 3) {
                                    talentPage.find('#next').text('开始超级穷鬼系统')
                                }
                            }
                        });
                    });
        ...
        <div id="main">
            <div class="head" style="font-size: 1.6rem">
                <div>调整初始穷鬼属性</div>
                <div id="total" style="font-size:1rem; font-weight:normal;">可用穷鬼属性点：0</div>
            </div>
            <ul id="propertyAllocation" class="propinitial"></ul>
            <ul class="selectlist" id="talentSelectedView"></ul>
            <div class="btn-area">
                <button id="start" class="mainbtn">开始超级穷鬼系统</button>
            </div>
        </div>
        `);
        ...
        const freshTotal = () => {
            propertyPage.find('#total').text(`可用穷鬼属性点：${this.#totalMax - total()}`);
        }
        ...
        groups.INT = getBtnGroups("天才值", 0, 10); // 天才值 intelligence INT
        groups.STR = getBtnGroups("体质", 0, 10); // 体质 strength STR
        groups.MNY = getBtnGroups("穷鬼币", 0, 10); // 穷鬼币 money MNY
        ...
        propertyPage
            ...
            .click(() => {
                // if (total() < this.#totalMax) {
                //     this.hint(`你还有${this.#totalMax - total()}穷鬼属性点没有分配完`);
                //     return;
                // } else 
                if (total() > this.#totalMax) {
                    this.hint(`你多使用了${total() - this.#totalMax}穷鬼属性点`);
                    return;
                }
        ...
        <div id="main">
            ...
            <div class="btn-area">
                ...
                <button id="summary" class="mainbtn">狗生总结</button>
                <button id="domToImage" class="mainbtn">狗生回放</button>
            </div>
            ...
            .click(() => {
                ...
                const li = $(`<li><span>${age}岁：</span><span>${content.map(
                    ({ type, description, grade, name, postEvent }) => {
                        switch (type) {
                            case 'TLT':
                                return `穷鬼天赋【${name}】发动：${description}`;
                            case 'EVT':
                ...
                <li><span>天才值</span><span>${property.INT}</span></li>
                <li><span>体质</span><span>${property.STR}</span></li>
                <li><span>穷鬼币</span><span>${property.MNY}</span></li>
        ...
        const summaryPage = $(`
        <div id="main">
            <div class="head">狗生总结</div>
            <ul id="judge" class="judge">
                <li class="grade2"><span>颜值：</span><span>9级 美若天仙</span></li>
                <li class="grade0"><span>天才值：</span><span>4级 天才值一般</span></li>
                <li class="grade0"><span>体质：</span><span>1级 极度虚弱</span></li>
                <li class="grade0"><span>穷鬼币：</span><span>6级 小康之家</span></li>
                <li class="grade0"><span>享年：</span><span>3岁 早夭</span></li>
                <li class="grade0"><span>快乐：</span><span></span>3级 不太幸福的狗生</li>
            </ul>
            <div class="head" style="height:auto;">穷鬼天赋，你可以选一个，下辈子还能抽到</div>
            ...
            achievement: {
                ...
                clear: () => {
                    ...
                    total.append(`
                        ...
                        <li class="achvg${getGrade('talentRate', talentRate)}"><span class="achievementtitle">穷鬼天赋收集率</span>${Math.floor(talentRate * 100)}%</li>
                    `);
                    ...
                    if (contents.length > 0)
                        $('#lifeTrajectory')
                            .append(`<li><span>初始：</span><span>${contents.map(
                                ({ source, target }) => `穷鬼天赋【${source.name}】发动：替换为穷鬼天赋【${target.name}】`
                            ).join('<br>')
                                }</span></li>`);
