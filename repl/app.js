...
`))
    ...
    Example:    /select 1 2 3 意味着选择 1 2 3 三个穷鬼天赋
    ...
    参数解释     /unselect <id1> [id2] [id3]
            ...
            case '/allocate':
            case '/attrib':
            case '/attribute': return `分配或查看穷鬼属性点
    ...
    s
    select
    /select         选择穷鬼天赋        /select <id1> [id2] [id3]
    ...
    /allocate
    /attrib
    /attribute      分配或查看穷鬼属性点 /allocate <TAG> <[+/-]value>
    ...
    talentSelect(...select) {
        ...
        for(const number of select) {
            ...
            if(!s) return warn(`${number} 为未知穷鬼天赋`);
            if(this.#talentSelected.has(s)) continue;
            if(this.#talentSelected.size == 3)
                return warn('你只能选3个穷鬼天赋。请使用 \x1B[4m/unselect\x1B[24m 取消选择你不想要的穷鬼天赋');
            ...
            if(exclusive != null)
                for(const { name, id } of this.#talentSelected)
                    if(id == exclusive)
                        return warn(`穷鬼天赋【${s.name}】与已选择的穷鬼天赋【${name}】冲突`);
    ...
    talentExtend(select) {
        ...
        const s = list[select];
        if(!s) return warn(`${select} 为未知穷鬼天赋`);
    ...
    list() {
        ...
        switch(this.#step) {
            case this.Steps.TALENT:
                description = '🎉 请选择（\x1B[4m/select\x1B[24m）3 个穷鬼天赋';
                list = this.#randomTalents;
                check = talent=>this.#talentSelected.has(talent);
                break;
            case this.Steps.SUMMARY:
                description = '🎉 你可以选（\x1B[4m/select\x1B[24m）一个穷鬼天赋继承';
    ...
    next(enter) {
        ...
        switch(this.#step) {
            case this.Steps.TALENT:
                if(this.#talentSelected.size != 3) return warn(this.list(), `请选择 3 个穷鬼天赋`);
            ...
            case this.Steps.PROPERTY:
                ...
                if(less > 0) return warn(this.prop(), `你还有 ${less} 穷鬼属性点没有分配完`);
    ...
    prop() {
        ...
        return `🎉 穷鬼属性分配
请使用 \x1B[4m/alloc\x1B[24m <TAG> <value> 分配穷鬼属性
剩余点数 ${this.less()}
...
穷鬼属性(TAG)       当前值
颜值(CHR)         ${property.CHR}
天才值(INT)         ${property.INT}
体质(STR)         ${property.STR}
穷鬼币(MNY)         ${property.MNY}
快乐(SPR)         ${property.SPR}`
    }
        ...
        switch(tag) {
            ...
            case 'MNY':
                ...
                if(
                    ...
                    || tempSet < 0
                ) return  warn('⚠ 不能分配负数穷鬼属性');
                if(tempSet>10) return  warn('⚠ 单项穷鬼属性最高分配10点');
